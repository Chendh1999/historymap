...
f=open(r'E:\develop_gis\map website\data\V6 Time Series Prefecture Polygons\v6_time_pref_pgn_utf_wgs84\v6_time_pref_pgn_utf_wgs84.txt','r',encoding='utf-8')
# 按行读入txt
flines=f.readlines()
for line in flines:
    # 去掉干扰，切词
    ...
    if (abbrlist[23]!=''):
        if(abbrlist[23].find('MULTI')>-1):
            ...
            geom= "st_geomfromtext('"+abbrlist[23].replace('POLYGON ((','MULTIPOLYGON (((').replace('))',')))') + "',4326)"
    # 拼接sql语句
    sqltxt="INSERT INTO v6_time_pref_pgn_utf_wgs84 (" \
